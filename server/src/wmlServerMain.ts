...
import { createConnection, IConnection, 
	TextDocuments, InitializeParams, 
	InitializeResult, RequestType, 
	DocumentRangeFormattingRequest, 
	Disposable, DocumentSelector 
} from 'vscode-languageserver';
import { DocumentContext } from 'vscode-wml-languageservice';
...
function validateTextDocument(textDocument: TextDocument): void {
	...
	if (textDocument.languageId === 'wml') {
...
connection.onCompletion(textDocumentPosition => {
	...
	if (mode && mode.doComplete) {
		if (mode.getId() !== 'wml') {
			connection.telemetry.logEvent({ key: 'wml.embbedded.complete', value: { languageId: mode.getId() } });
		}
