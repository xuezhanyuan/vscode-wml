...
suite('WML Completion', () => {
	...
	let testCompletionFor = function (value: string, expected: { count?: number, items?: ItemDescription[] }): PromiseLike<void> {
		...
		let list = ls.doComplete(document, position, wmlDoc);
		if (expected.count) {
	...
	test('Complete wml', function (testDone): any {
		run([
			testCompletionFor('<|', {
				items: [
					{ label: 'data', resultText: '<data' },
					{ label: 'dataset', resultText: '<dataset' },
				]
			})
		], testDone);
	});
