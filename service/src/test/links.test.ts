...
suite('WML Link Detection', () => {

	function getDocumentContext(documentUrl: string): wmlLanguageService.DocumentContext {
		return {
	...
	function testLinkCreation(modelUrl: string, tokenContent: string, expected: string): void {
		let document = TextDocument.create(modelUrl, 'wml', 0, `<a href="${tokenContent}">`);
