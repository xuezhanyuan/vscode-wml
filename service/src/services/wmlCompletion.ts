...
export function doComplete(document: TextDocument, position: Position, wmlDocument: WMLDocument): CompletionList {
	...
	let node = wmlDocument.findNodeBefore(offset);
	if (!node) {
		return result;
	}
	...
	function collectOpenTagSuggestions(afterOpenBracket: number, tagNameEnd?: number): CompletionList {
		...
		tagProviders.forEach((provider) => {
			provider.collectTags((tag, label,plable) => {

				if(provider.getId()==='html'){
					result.items.push({
						label: tag,
